...
// Reads the live feed, scores it with the pool's rules, and writes one Supabase row
// containing: m (group results), ko (team knockout stage), koFix (knockout fixtures for
// the Matchday calendar), locked (manual overrides), updatedAt.
// Env: SUPABASE_URL, SUPABASE_SERVICE_KEY, FOOTBALL_DATA_KEY (optional -> openfootball fallback)

const SUPABASE_URL=process.env.SUPABASE_URL;
const SERVICE_KEY=process.env.SUPABASE_SERVICE_KEY;
const FD_KEY=process.env.FOOTBALL_DATA_KEY||"";
const ROWS=["wc2026","wc2026_b"];   // pool 1 and pool 2 share one feed
...
// KO round -> stage index reached on a win (R16=2,QF=3 wait see mapping below)
// (matches site STAGES: reaching R16=2->3pts, QF=3->6, SF=4->9, Final=5->12, Champion=6->16)
function reachedIndex(round){const r=(round||"").toUpperCase();
  if(r.includes("THIRD")||r.includes("3RD"))return null;
  ...
  return null;}
function stageLabel(round){const r=(round||"").toUpperCase();
  ...
  if(r==="FINAL"||r.includes("THE FINAL"))return "Final";
  if(r.includes("32"))return "Round of 32";
  if(r.includes("16"))return "Round of 16";
  return round||"Knockout";}
const isRealTeam=n=>!!n && !/(winner|runner|tbd|placeholder|\bpath\b|loser|to be|group\s)/i.test(n);

// ---- feed adapters -> normalized items ----
// {scope:'group'|'ko', group?, round?, iso, home, away, finished, winner, sh, sa}
async function fromFootballData(){
  ...
  const j=await res.json();const items=[];
  for(const mt of (j.matches||[])){
    const finished=mt.status==="FINISHED"||mt.status==="AWARDED";
    const w=mt.score&&mt.score.winner;
    const winner=w==="HOME_TEAM"?"home":w==="AWAY_TEAM"?"away":w==="DRAW"?"draw":null;
    const ft=(mt.score&&mt.score.fullTime)||{};
    const iso=(mt.utcDate||"").slice(0,10);
    const home=mt.homeTeam&&mt.homeTeam.name,away=mt.awayTeam&&mt.awayTeam.name;
    const stage=mt.stage||"";
    if(stage.includes("GROUP")){
      const group=(mt.group||"").replace(/GROUP[_ ]?/i,"").trim().charAt(0).toUpperCase();
      items.push({scope:"group",group,iso,home,away,finished,winner});
    }else{
      items.push({scope:"ko",round:stageLabel(stage),iso,home,away,finished,winner,
        sh:finished?ft.home:null,sa:finished?ft.away:null});
    }
  }
  return items;
}
async function fromOpenfootball(){
  ...
  const j=await res.json();const items=[];
  for(const mt of (j.matches||[])){
    const sc=mt.score||{};const ft=sc.ft;const finished=Array.isArray(ft)&&ft.length>=2;
    const pen=sc.p||sc.pen,et=sc.et;let winner=null;
    if(finished){
    ...
    const iso=(mt.date||"").slice(0,10);
    const isGroup=/^matchday/i.test(mt.round||"");
    if(isGroup){
      const group=(mt.group||"").replace(/group[_ ]?/i,"").trim().charAt(0).toUpperCase();
      items.push({scope:"group",group,iso,home:mt.team1,away:mt.team2,finished,winner});
    }else{
      items.push({scope:"ko",round:stageLabel(mt.round),iso,home:mt.team1,away:mt.team2,finished,winner,
        sh:finished?ft[0]:null,sa:finished?ft[1]:null});
    }
  }
  return items;
}
...
function buildScores(items){
  ...
  for(const e of items){
    ...
    if(e.scope==="group"){
      ...
      else if(e.winner){const win=resolve(e.winner==="home"?e.home:e.away);m[f.id]=win===f.a?"A":"B";}
    }else{
      const reached=reachedIndex(e.round);if(reached==null||!e.winner||e.winner==="draw")continue;
...
// knockout fixtures for the Matchday calendar (only once real teams are set)
function buildKoFix(items){
  const out=[];
  for(const e of items){
    if(e.scope!=="ko")continue;
    if(!isRealTeam(e.home)||!isRealTeam(e.away))continue;
    out.push({iso:e.iso,round:e.round,home:resolve(e.home)||e.home,away:resolve(e.away)||e.away,
      winner:e.finished?e.winner:null,
      sh:(e.finished&&e.sh!=null)?e.sh:null,sa:(e.finished&&e.sa!=null)?e.sa:null});
  }
  out.sort((a,b)=>(a.iso||"").localeCompare(b.iso||""));
  return out;
}

async function getCurrent(row){
...
async function putRow(row,data){
...
(async function main(){
  let items;
  try{items=FD_KEY?await fromFootballData():await fromOpenfootball();}
  catch(e){console.error("Primary feed failed:",e.message);
    if(FD_KEY){console.error("Falling back to openfootball…");items=await fromOpenfootball();}else throw e;}
  const {m:fm,ko:fko}=buildScores(items);
  const koFix=buildKoFix(items);
  for(const row of ROWS){
    const cur=await getCurrent(row);const locked=cur.locked||{};
    const m={...fm},ko={...fko};
    ...
    await putRow(row,{m,ko,koFix,locked,updatedAt:Date.now()});
    console.log(`[${row}] ${Object.keys(m).length} group + ${Object.keys(ko).length} KO scores, ${koFix.length} KO fixtures via ${FD_KEY?"football-data.org":"openfootball"}.`);
  }
